...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            logger.info(f"bexists value:  {bexists}")
            load_on_all_ranks = (
            ...
            logger.info(f"load on all ranks value:  {load_on_all_ranks}")
            ...
            try:
                if self.cfg.checkpoint.use_ema_weights_to_init_param and "extra_state" in state and "ema" in state["extra_state"]:
                    ...
                    logger.info(f"loading in if block")
                else:
                    self.model.load_state_dict(
                        state["model"], strict=True, model_cfg=self.cfg.model
                    )
                    logger.info(f"loading in else block")
        ...
        if last_optim_state is not None and not reset_optimizer:
            logger.info(f"Reloading optims")
