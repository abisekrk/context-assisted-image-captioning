...
class ConsistencyDetectionDataset(OFADataset):
    def __init__(
        self,
        split,
        dataset,
    ...
    def __getitem__(self, index):
        try:
            uniq_id, image, caption, description, label = self.dataset[index]
        except IndexError:
            print("Invali Index: ", index)
        if str(label) == '0':
            label = 'no'
        elif str(label) == '1':
            label = 'yes'
        ...
        src_item = self.encode_text('is the text " {} " consistent with the image  and the text " {} " ?'.format(tgt_caption, tgt_description))
