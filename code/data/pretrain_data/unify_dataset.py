...
class UnifyDataset(OFADataset):
    ...
    def process_image_text_pair(self, index):
        ...
        # For consistency detection, question will be context, refs will be the label
        context = question
        label = refs

        
        image = image.replace('"','')
        image_path = self.image_base_path + image[1:]
        ...
        if type == 'caption':
        ...
        elif type == 'context_caption':
            tgt_caption = self.pre_caption(caption, self.max_tgt_length)
            tgt_description = self.pre_caption(context, self.max_src_length)
            pos_src_caption = self.pre_caption(caption, self.max_tgt_length)
            neg_src_caption = self.pre_caption(self.get_negative_caption(caption, gt_objects), self.max_tgt_length)
            # src_item = self.encode_text(" what does the image describe?")

            src_item = self.encode_text('what does the image describe based on the text " {} " ?'.format(tgt_description))[0:self.max_src_length]
            tgt_item = self.encode_text(" {}".format(tgt_caption))
            pos_src_item = self.encode_text(' does the image describe " {} " based on the text  " {} " ?'.format(pos_src_caption, tgt_description))[0:self.max_src_length]
            neg_src_item = self.encode_text(' does the image describe " {} " based on the text  " {} " ?'.format(neg_src_caption, tgt_description))[0:self.max_src_length]

        elif type == 'consistency_detection':
            tgt_caption = self.pre_caption(caption, self.max_tgt_length)
            tgt_description = self.pre_caption(context, self.max_src_length)
            pos_src_caption = self.pre_caption(caption, self.max_tgt_length)
            neg_src_caption = self.pre_caption(self.get_negative_caption(caption, gt_objects), self.max_tgt_length)
            ...
            src_item = self.encode_text('is the text " {} " consistent with the image  and the text " {} " ?'.format(tgt_caption, tgt_description))[0:self.max_src_length]
            tgt_item = self.encode_text(" {}".format(label))

            pos_src_item = self.encode_text('is the text " {} " consistent with the image  and the text " {} " ?'.format(pos_src_caption, tgt_description))[0:self.max_src_length]
            neg_src_item = self.encode_text('is the text " {} " consistent with the image  and the text " {} " ?'.format(neg_src_caption, tgt_description))[0:self.max_src_length]
        ...
        # neg_src_item = torch.cat([self.bos_item, neg_src_item, self.eos_item]) if (type != 'visual_grounding' and type != 'consistency_detection') else None
    ...
    def process_pure_text(self, index):
        ...
        examples = []
        for _ in range(2):
            # res = self.pure_text_dataset[index]
            # print("\n\n\n\n")
            # print(res)
            uniq_id, text, keyword = self.pure_text_dataset[index]
            text = text.strip().lower()
            arr = keyword.split(";")
            keyword = " ".join(arr)

            # text_item = self.encode_text(" {}".format(text), length=512)
            prompt = ' what are the keywords in the article " {} "?'
            ...
            tgt_item = self.encode_text('{}'.format(keyword))
            noise_tgt_item = self.add_noise_to_tgt(tgt_item.clone(), 0.0)
