...
def eval_caption(task, generator, models, sample, **kwargs):
    ...
    for i, sample_id in enumerate(sample["id"].tolist()):
        # print("\n\n\n\n\n\n", hypos)
        if hypos[i] and hypos[i][0] and (hypos[i][0]["tokens"] is not None):
            detok_hypo_str = decode_fn(hypos[i][0]["tokens"], task.tgt_dict, task.bpe, generator)
            results.append({"image_id": int(sample_id), "caption": detok_hypo_str.translate(transtab).strip()})
    return results, None
...
def eval_step(task, generator, models, sample, **kwargs):
    # print("\n\n\n\n\nTask name: ", task.cfg._name)
    if task.cfg._name == 'caption' or task.cfg._name == 'context_caption' or task.cfg._name == 'context_caption_ne':
    ...
    elif task.cfg._name == 'snli_ve' or task.cfg._name == 'consistency_detection':
    ...
    elif task.cfg._name == 'gigaword' or task.cfg._name == 'keyword_extraction':
